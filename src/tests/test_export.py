...
from src.cmdb_exchange.builders import EnvironmentUsersFile, MasterUsersFile, \
    CmdbDataFile
...
def test_create_exporter_with_unknown_format():
    with pytest.raises(UnsupportedFormat) as error:
        CmdbExchange.create_exporter('zzz', EnvironmentUsersFile())
...
def test_create_environment_users_file(tmpdir,
    ...
    exporter = CmdbExchange.create_exporter('csv', EnvironmentUsersFile())
...
def test_create_master_users_file(tmpdir,
    ...
    exporter = CmdbExchange.create_exporter('csv', MasterUsersFile())
    exporter.export(tmpdir, python_nested_data)
...
def test_create_cmdb_download_file(tmpdir,
                                  python_nested_data,
                                  cmdb_data_csv_file):
    exporter = CmdbExchange.create_exporter('csv', CmdbDataFile())
    exporter.export(tmpdir, python_nested_data)
